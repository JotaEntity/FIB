...
/* Para atualizar do meu código e aceitar + de 1 padaria foi inserir um scan, e apresentar a padaria em que ele estava */
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("Insira o nome da Padaria em que você irá hoje:");
        String namePadaria = sc.next();
        System.out.println("Bem vindo a padaria " + namePadaria);
            ...
            if (dado == 2){
                ...
                if (newvalue == 1){
                    if (padariaUser.checkLimitInBread() == false){
                        ...
                        int next = sc.nextInt();
                        if (next == 1){
                            System.out.println("Compra finalizada, no valor de R$" + padariaUser.checkLimitInBread() + " na padaria: " + namePadaria);
                            sc.close();
                        } else {
                        ...
                        if (padariaUser.checkLimitInBread() == true){
                            ...
                            if (menunewnext == 1){
                                if (padariaUser.checkLimitInBread() == true) {
                                    ...
                                    int v = sc.nextInt();
                                    if (v == 1){
                                        System.out.println("Compra finalizada no valor de R$: " + padariaUser.returnValueFromProducts() + " na padaria: " + namePadaria);
                                        sc.close();
                                    } else if (v == 2){
                                        if (padariaUser.checkLimitInBread() == false){
                                            System.out.println("Você não pode mais adicionar pão ao seu carrinho, finalize a compra.");
